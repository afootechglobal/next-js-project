...
import { isValidEmail, generateOtp,convertToTitleCase } from './dbConfig/function';
import {getBackendSettingsDetails } from './dbConfig/mail/mailer'
  ...
  try {
    ...
    if (email == '') {
      ...
      if (!isValidEmail(email)) {
        ...
        const [getUser] = await connection.query('SELECT * FROM user_tab WHERE email = ?', [email]);

        if (getUser.length > 0) {

          const statusId = getUser[0].status_id;
          const userId = getUser[0].user_id;
          const dbfullname = getUser[0].fullname;
          const fullname = convertToTitleCase(dbfullname);
          ...
          if (statusId == 1) {
            ...
            // const [smtp_host, smtp_username, smtp_password, smtp_port, sender_name]= await getBackendSettingsDetails(connection, 'BK_ID001');
            await getBackendSettingsDetails(connection, 'BK_ID001', otp, email, fullname, 1)
